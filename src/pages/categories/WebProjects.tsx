...
const WebProjects = () => {
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim() !== '') {
      ...
      webProjects = webProjects.filter(project => 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="flex items-center justify-center gap-4 mb-6">
              <div className="w-16 h-16 rounded-full bg-purple-500 bg-opacity-20 flex items-center justify-center">
                <FaCode className="text-3xl text-purple-500" />
              </div>
            </div>
            
            <h1 className="text-5xl font-bold text-white mb-6">Desenvolvimento Web</h1>
            <p className="text-secondary text-lg max-w-3xl mx-auto">
              Projetos de desenvolvimento web focados em criação de interfaces, aplicações web interativas
              e integrações com sistemas industriais, utilizando tecnologias modernas como React, TypeScript e Node.js.
            </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="flex flex-col md:flex-row gap-4 items-start mb-6">
              ...
              <div className="relative w-full md:w-1/2">
                <input
                  type="text"
                  placeholder="Pesquisar projetos web..."
                  value={searchTerm}
                ...
                {selectedTags.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {selectedTags.map(tag => (
                      <span key={tag} className="inline-flex items-center gap-1 px-3 py-1 bg-purple-500 bg-opacity-20 rounded-full text-xs text-purple-400">
                        {tag}
                        <button onClick={() => handleTagToggle(tag)} className="text-purple-400 hover:text-white">
                          <FaTimes size={10} />
                        </button>
                      </span>
                    ))}
          ...
          {filteredProjects.length > 0 ? (
            ...
            >
              ...
              <button
                onClick={clearFilters}
                className="py-2 px-6 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors"
              >
                Limpar filtros
              </button>
            </motion.div>
