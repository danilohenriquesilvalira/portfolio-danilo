...
import { FaSearch, FaTimes, FaArrowLeft, FaRobot } from 'react-icons/fa';
...
const Industry40Projects = () => {
  const [filteredProjects, setFilteredProjects] = useState<Project[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedTags, setSelectedTags] = useState<string[]>([]);
  
  // Filtrar projetos para mostrar apenas a categoria Indústria 4.0
  useEffect(() => {
    let industry40Projects = projects.filter(p => p.category === 'industria40');
    
    // Filtrar por termo de pesquisa
    if (searchTerm.trim() !== '') {
      const term = searchTerm.toLowerCase();
      industry40Projects = industry40Projects.filter(project => 
    ...
    setFilteredProjects(industry40Projects);
  }, [searchTerm, selectedTags]);
  
  // Obter tags únicas apenas de projetos de Indústria 4.0
  const industry40Tags = Array.from(new Set(
    projects
      .filter(p => p.category === 'industria40')
      ...
      <section className="bg-gradient-to-b from-primary to-black-100 pt-32 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <h1 className="text-5xl font-bold text-white mb-6">Indústria 4.0</h1>
            <p className="text-secondary text-lg max-w-3xl mx-auto">
              Projetos focados em tecnologias da Indústria 4.0, incluindo Internet das Coisas Industrial (IIoT),
              análise de dados em tempo real, Digital Twin, sistemas ciberfísicos e soluções de conectividade avançada.
            </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="flex flex-col md:flex-row gap-4 items-start mb-6">
              ...
              <div className="relative w-full md:w-1/2">
                <input
                  type="text"
                  placeholder="Pesquisar projetos de Indústria 4.0..."
                  value={searchTerm}
            ...
            <div className="flex justify-between items-center">
              <div className="text-secondary">
                <span>Exibindo {filteredProjects.length} de {projects.filter(p => p.category === 'industria40').length} projetos</span>
                
                {/* Tags selecionadas */}
                {selectedTags.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {selectedTags.map(tag => (
                      <span key={tag} className="inline-flex items-center gap-1 px-3 py-1 bg-green-500 bg-opacity-20 rounded-full text-xs text-green-400">
                        {tag}
                        <button onClick={() => handleTagToggle(tag)} className="text-green-400 hover:text-white">
                          <FaTimes size={10} />
                        </button>
                      </span>
                    ))}
          ...
          {filteredProjects.length > 0 ? (
            ...
            >
              ...
              <button
                onClick={clearFilters}
                className="py-2 px-6 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors"
              >
                Limpar filtros
              </button>
            </motion.div>
...
export default Industry40Projects;
