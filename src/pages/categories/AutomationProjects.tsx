...
import { FaSearch, FaTimes, FaArrowLeft, FaIndustry } from 'react-icons/fa';
...
const AutomationProjects = () => {
  ...
  // Filtrar projetos para mostrar apenas a categoria de automação
  useEffect(() => {
    let automationProjects = projects.filter(p => p.category === 'automacao');
    
    // Filtrar por termo de pesquisa
    if (searchTerm.trim() !== '') {
      const term = searchTerm.toLowerCase();
      automationProjects = automationProjects.filter(project => 
    ...
    setFilteredProjects(automationProjects);
  }, [searchTerm, selectedTags]);
  
  // Obter tags únicas apenas de projetos de automação
  const automationTags = Array.from(new Set(
    projects
      .filter(p => p.category === 'automacao')
          ...
          >
            <div className="flex items-center justify-center gap-4 mb-6">
              <div className="w-16 h-16 rounded-full bg-blue-500 bg-opacity-20 flex items-center justify-center">
                <FaIndustry className="text-3xl text-blue-500" />
              </div>
            </div>
            
            <h1 className="text-5xl font-bold text-white mb-6">Automação Industrial</h1>
            <p className="text-secondary text-lg max-w-3xl mx-auto">
              Projetos de automação industrial desenvolvidos com foco em otimização de processos, 
              controle e monitoramento em tempo real utilizando PLCs, HMIs e sistemas SCADA.
            </p>
          ...
          >
            <div className="flex flex-col md:flex-row gap-4 items-start mb-6">
              ...
              <div className="relative w-full md:w-1/2">
                <input
                  type="text"
                  placeholder="Pesquisar projetos de automação..."
                  value={searchTerm}
            ...
            <div className="flex justify-between items-center">
              <div className="text-secondary">
                <span>Exibindo {filteredProjects.length} de {projects.filter(p => p.category === 'automacao').length} projetos</span>
                
                {/* Tags selecionadas */}
                {selectedTags.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {selectedTags.map(tag => (
                      <span key={tag} className="inline-flex items-center gap-1 px-3 py-1 bg-tech-blue bg-opacity-20 rounded-full text-xs text-tech-blue">
                        {tag}
                        <button onClick={() => handleTagToggle(tag)} className="text-tech-blue hover:text-white">
                          <FaTimes size={10} />
                        </button>
                      </span>
                    ))}
              ...
              {(searchTerm || selectedTags.length > 0) && (
                <button
                  ...
                  className="flex items-center gap-2 text-tech-blue hover:text-white transition-colors"
                >
                  <FaTimes size={12} />
                  Limpar filtros
                </button>
